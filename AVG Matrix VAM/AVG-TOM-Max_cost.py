	...
	for i in supply:
		for j in demand:
			if costs[i][j]!=0:
				...
				avg=avg+ costs[i][j] 
	l=len(supply)- (len(supply)%10)
	b= len(demand)- (len(demand)%10)
	avg = avg /(l* b)
	
	for i in supply:
		for j in demand:
			if costs[i][j]!=0:
				costs[i][j]=costs[i][j]/avg
...
with open('AVG-TOM-VOM_maximum_penalty_cost.csv', 'w', encoding="ISO-8859-1", newline='') as myfile:
