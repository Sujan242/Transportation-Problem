...
for instance in range(640):
	...
	while g:
	    ...
	    for x in demand:
	        d[x] = (costs1[g[x][1]][x] - costs1[g[x][0]][x]) if len(g[x]) > 1 else (costs1[g[x][0]][x])
	    s = {}
	    for x in supply:
	        s[x] = (costs1[x][g[x][1]] - costs1[x][g[x][0]]) if len(g[x]) > 1 else costs1[x][g[x][0]]
	    ...
	    if(kd):
	        ...
	        if(d[fm]>s[t]):
	            ...
	            if( d[fm]*costs1[f2][t2] > m1):
	                tfin , fin = t2,f2
	                m1=costs1[f2][t2]*d[fm]
...
with open('VOM_maximum_penalty_cost.csv', 'w', encoding="ISO-8859-1", newline='') as myfile:
