...
for instance in range(640):
	...
	while g:
	    ...
	    for x in demand:
	        d[x] = ((costs[g[x][1]][x] - costs[g[x][0]][x])* min(demand[x],supply[g[x][0]]))/(1.0*demand[x]) if len(g[x]) > 1 else ((costs[g[x][0]][x])*min(demand[x],supply[g[x][0]]))/(1.0*demand[x])
	    s = {}
	    for x in supply:
	        d[x] = ((costs[x][g[x][1]] - costs[x][g[x][0]])*min(supply[x],demand[g[x][0]]))/(1.0*supply[x]) if len(g[x]) > 1 else (costs[x][g[x][0]]*min(supply[x],demand[g[x][0]]))/(1.0*supply[x])
...
with open('Weighted-cost-method-TOM-unit-penalty.csv', 'w', encoding="ISO-8859-1", newline='') as myfile:
